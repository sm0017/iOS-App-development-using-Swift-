...
class CalculatorViewController: UIViewController {
    ...
    */
    ...
    // When the segue is ready we assign the brain.program to graphViewController program so that it can acts as delegate of datasource for GraphView
    // We also set the title of graph 
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        var destination = segue.destinationViewController as UIViewController
       
        // if it is naviation controller
        if let navCtrl = destination as? UINavigationController {
            destination = navCtrl.visibleViewController!
        }
        // if its is GraphViewController
        if let graphctrl = destination as? GraphViewController {
            if let identifier = segue.identifier {
                switch identifier {
                 case "plot graph":
                 graphctrl.program = brain.program
                 
                 if brain.description != "" {
                 graphctrl.title = brain.description.componentsSeparatedByString(", ").last
                 }else {
                 graphctrl.title = "unknown function"
                 }
